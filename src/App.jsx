  ...
  const shareMyList = () => {
    ...
    if (navigator.share) {
      navigator.share({
        ...
        text: 'Découvre les jeux que j\'ai repérés pour le FIJ !',
        url: shareUrl,
      });
    } else {
      ...
      <header className="bg-white border-b sticky top-0 z-50 px-4 py-4 shadow-sm">
        <div className="max-w-7xl mx-auto">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2">
              ...
              <h1 className="text-xl font-black uppercase italic tracking-tighter">
                FIJ <span className="text-indigo-600">2026</span>
              </h1>
            </div>
            <div className="flex gap-2">
              <button onClick={shareMyList} className="bg-slate-100 text-slate-600 p-2.5 rounded-xl hover:bg-indigo-50 hover:text-indigo-600 transition-all">
                <Share2 size={20} />
              </button>
          ...
          <div className="relative mb-4">
            ...
            <input 
              type="text" 
              placeholder="Titre, auteur, stand..." 
              className="w-full pl-11 pr-12 py-3 bg-slate-100 rounded-2xl outline-none focus:ring-2 ring-indigo-500/20" 
              value={searchTerm} 
              onChange={(e) => setSearchTerm(e.target.value)} 
            />
            {searchTerm && (
              <button onClick={() => setSearchTerm('')} className="absolute right-4 top-1/2 -translate-y-1/2 text-slate-400"><X size={18}/></button>
            )}
          </div>

          <div className="flex flex-wrap gap-x-4 gap-y-2 border-b border-slate-100 pb-1">
            {[
              {id:'mylist', label:'Likes', icon:Heart}, 
              ...
              <button 
                ...
                className={`flex items-center gap-2 pb-2 border-b-2 font-black text-[10px] uppercase transition-all whitespace-nowrap ${activeTab === t.id ? 'border-indigo-600 text-indigo-600' : 'border-transparent text-slate-400'}`}
              >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {displayData.finalItems.map(game => (
            <div 
              ...
              className={`bg-white rounded-[1.5rem] border p-5 flex flex-col hover:shadow-xl transition-all cursor-pointer group ${tested.includes(game.id) ? 'bg-green-50/50 border-green-200' : 'border-slate-100'}`}
            >
              <div className="flex justify-between mb-1">
                <h3 className="font-black text-lg leading-tight flex-1 group-hover:text-indigo-600">{game.title}</h3>
                <button 
                  onClick={(e) => { e.stopPropagation(); toggleFavorite(game.id); }} 
                  className={`ml-2 transition-transform active:scale-125 ${favorites.includes(game.id) ? 'text-red-500' : 'text-slate-300'}`}
                >
              ...
              <div className="mt-auto pt-3 border-t flex justify-between items-center gap-2">
                <span className="text-[10px] font-black uppercase text-slate-400 truncate min-w-0">
                  {game.publisher}
                </span>
                <button 
                  onClick={(e) => { e.stopPropagation(); toggleTested(game.id); }} 
                  className={`px-3 py-1.5 rounded-xl text-[10px] font-black uppercase flex items-center gap-1.5 transition-all flex-shrink-0 whitespace-nowrap ${
                    tested.includes(game.id) ? 'bg-green-600 text-white shadow-md' : 'bg-slate-100 text-slate-400 hover:bg-slate-200'
                  }`}
                >
          ...
          <div className="bg-white w-full max-w-lg rounded-[2.5rem] p-8 relative animate-in zoom-in-95" onClick={e => e.stopPropagation()}>
            <div className="flex justify-between items-start mb-6">
              ...
              <div className="flex gap-2">
                <button 
                  ...
                  className={`p-2 rounded-full transition-all active:scale-125 ${favorites.includes(selectedGame.id) ? 'bg-red-50 text-red-500' : 'bg-slate-100 text-slate-400'}`}
                >
            ...
            <div className="grid grid-cols-3 gap-2 mb-6">
              <div className="bg-slate-50 p-3 rounded-2xl text-center border border-slate-100">
                <Users size={16} className="mx-auto mb-1 text-indigo-600"/><span className="text-[10px] font-black block">{selectedGame.players} j.</span>
              </div>
              <div className="bg-slate-50 p-3 rounded-2xl text-center border border-slate-100">
                <Clock size={16} className="mx-auto mb-1 text-indigo-600"/><span className="text-[10px] font-black block">{selectedGame.duration}</span>
              </div>
              <div className="bg-amber-50 p-3 rounded-2xl text-center border border-amber-100">
                <MapPin size={16} className="mx-auto mb-1 text-amber-600"/><span className="text-[10px] font-black block text-amber-700 uppercase">St {selectedGame.stand}</span>
              </div>
            </div>
            ...
            <div className="bg-slate-50 p-5 rounded-3xl mb-8 relative border border-slate-100 shadow-inner">
              ...
              <div className="text-sm text-slate-700 leading-relaxed max-h-56 overflow-y-auto pr-2 custom-scrollbar italic font-medium">
            ...
            <div className="grid grid-cols-2 gap-4">
              <a 
                href={selectedGame.myludoUrl} 
                target="_blank" 
                rel="noreferrer" 
                className="bg-[#364958] text-white py-4 rounded-2xl text-[10px] font-black text-center uppercase tracking-widest flex items-center justify-center gap-2 shadow-lg active:scale-95 transition-all"
              >
                <ExternalLink size={14}/> MyLudo
              </a>
              <a 
                href={selectedGame.bggUrl} 
                target="_blank" 
                rel="noreferrer" 
                className="bg-[#FF5100] text-white py-4 rounded-2xl text-[10px] font-black text-center uppercase tracking-widest flex items-center justify-center gap-2 shadow-lg active:scale-95 transition-all"
              >
                <ExternalLink size={14}/> BGG
              </a>
            </div>
          </div>
        </div>
